...
def main():
    ...
    print("\n[3/5] Setting up Stratified K-Fold based on price quantiles...")
    n_folds = 5
    ...
    print(f"\nOverall OOF Neural Adapter SMAPE: {smape(y_train, oof_adapter):.2f}%")
    ...
    try:
        ...
        print("CatBoost GPU acceleration confirmed active!")
    except Exception:
        print("CatBoost GPU fallback to multi-threaded CPU.")
        catboost_task_type = "CPU"

    print("\nTraining GBDT Models (LightGBM & CatBoost) across folds...")
    for fold, (t_idx, v_idx) in enumerate(cv_splits):
        print(f"\n--- Fold {fold + 1}/{n_folds} ---")
        X_tr, y_tr_log = X_train_gbdt[t_idx], y_log[t_idx]
        X_va, y_va_log = X_train_gbdt[v_idx], y_log[v_idx]
        ...
        test_ridge += np.maximum(np.exp(m_ridge.predict(X_test_gbdt)), 0.01) / n_folds
        ...
        m_lgbm.fit(
            ...
            callbacks=[lgb.early_stopping(80, verbose=False), lgb.log_evaluation(period=100)],
        )
        val_pred_lgbm = np.maximum(np.exp(m_lgbm.predict(X_va)), 0.01)
        oof_lgbm[v_idx] = val_pred_lgbm
        test_lgbm += np.maximum(np.exp(m_lgbm.predict(X_test_gbdt)), 0.01) / n_folds
        print(f"  [LightGBM] Fold {fold+1} SMAPE: {smape(y_va_true, val_pred_lgbm):.2f}%")
        ...
        test_cat += np.maximum(np.exp(m_cat.predict(X_test_gbdt)), 0.01) / n_folds
        print(f"  [CatBoost] Fold {fold+1} SMAPE: {smape(y_va_true, val_pred_cat):.2f}%")
    ...
    print(f"Overall OOF Neural Adapter SMAPE:{smape(y_train, oof_adapter):.2f}%")

    # 6. Out-of-Fold Nelder-Mead Blending
    print("\n[5/5] Optimizing 4-Way Ensemble Weights via Nelder-Mead directly on SMAPE...")
    oof_dict = {
        "adapter": oof_adapter,
        "lgbm": oof_lgbm,
        "cat": oof_cat,
    ...
    print(f"\n=======================================================")
    print(f"  >>> FINAL 4-WAY BLENDED ENSEMBLE OOF SMAPE: {final_smape:.2f}% <<<")
    print(f"=======================================================")

    # Generate and validate test predictions
    test_dict = {
